...
if (mysqli_num_rows($result) > 0) {
	...
	while($row = mysqli_fetch_assoc($result)) {
		...
		if ($row['needs_permit'] == 1) {
			if(isset($_SESSION['isloggedin'])) {
				if($_SESSION['isloggedin'] == True) {
					$needs_permit = '<i class="fas fa-lock permitlock" style="color:#44A601;top:'.$needs_permit_height.';"></i>';
					$ok_to_operate = True;
				}else{
					$needs_permit = '<i class="fas fa-lock permitlock" style="top:'.$needs_permit_height.';"></i>';
				}
			}else{
				$needs_permit = '<i class="fas fa-lock permitlock" style="top:'.$needs_permit_height.';"></i>';
			}
		...
		if ($row['needs_confirmation'] == True) {
			$confirm_icon = '<i class="fas fa-window-restore confirmation" style="top:'.$confirm_icon_heigt.';"></i>';
		}
